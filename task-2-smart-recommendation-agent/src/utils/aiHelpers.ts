...
export const generateQuery = async (user_persona: string, userQuery: string) => {
    const queryResponse = await openaiClient.chat.completions.create({
        model,
        messages: [
            {
                role: "system",
                content: "You are an expert in generating queries for a vector database to retrieve relevant information."
            },
            {
                role: "user",
